function saveCorrectedImageAndAnnotations(img, features, savepath)
    ...
    timestamp = string(datetime('now', 'Format', 'ddMMyyyy_HHmmssSSS')); % Formato: ddMMyyyy_HHmmssSSS
    ...
    correctedImageFileName = fullfile(savepath, strcat('corrected_image_', timestamp, '.png'));
    imwrite(img, correctedImageFileName);

    maskFileName = fullfile(savepath, strcat('corrected_mask_', timestamp, '.png'));
    imwrite(fullBW, maskFileName);

    % Crear y guardar las anotaciones en formato YOLO
    [height, width, ~] = size(img);
    annotationFileName = fullfile(savepath, strcat('annotations_', timestamp, '.txt'));
