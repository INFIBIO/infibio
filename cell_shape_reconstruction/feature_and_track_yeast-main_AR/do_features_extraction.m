...
%haploid/diploid, sporulated and else. "features(Y).DetectedClass" give the
%class of the detected bbx. Classes are: 
%1 = haploid/diploid
%2 = sporulated
%3 = else
...
%   2 July, 2024: AR. Modified. Implemented run_yolo_img.m function to
...
predict_clas_script = 'C:\Users\uib\Nextcloud2\classification_model\yolov5\classification\predict.py'; % predict_script: path to predict.py script in yolov5 cloned folder.
...
for tt = 1:length(myfiles)
    ...
    features = classification(img, features, python_path, predict_script, weights, yolo_path);
