function countsTable = count_single_cells(table, value, path)
    ...
    uniqueTemp = unique(table.Temperature);
 % Initialize empty array of structs
    countsTable = struct('count', [],'Time', [], 'Well', [], 'Replica', [], 'Zymolyase', [], 'Concentration', [], 'Temperature', []);
            ...
            for h = 1:length(uniqueW)
                 for j = 1:length(uniqueR)
                     for e = 1:length(uniqueC)
                         for f = 1:length(uniqueTemp)
                    % Filter the table based on current 'Time', 'Well', 'Zymolyase' and 'Replica' values
                    filteredTable = table(table.Time == uniqueT(i) & table.Well == uniqueW(h) & table.Replica == uniqueR(j) & table.Zymolyase == uniqueZ(k) & table.Concentration == uniqueC(e) & table.Temperature == uniqueTemp(f), :);
                    ...
                    if count > 0
                        ...
                        newRow = struct('count', count, 'Time', uniqueT(i), 'Well', uniqueW(h), 'Replica', uniqueR(j), 'Zymolyase', uniqueZ(k), 'Concentration', uniqueC(e), 'Temperature', uniqueTemp(f));
    ...
    groupedData = groupsummary(csvTable, {'Time', 'Zymolyase', 'Concentration', 'Temperature'}, {'mean', 'std'}, 'count');
