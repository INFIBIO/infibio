function [features] = classification(img, props, python_path, yolo_path, predict_class_script, weights_class)
...
% predict_script, weights as arguments to the function.
    ...
    for k = 1:length(props)
        ...
        python_cmd = sprintf('%s %s --weights %s --img %d --source %s --save-txt', python_path, predict_class_script, weights_class, img_size,  temp_img_path);
